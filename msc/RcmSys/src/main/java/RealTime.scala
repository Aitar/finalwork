...
object ConnHelper extends Serializable {
  // 懒变量定义，使用的时候才初始化
  lazy val jedis = new Jedis("192.168.10.102")
  lazy val mongoClient: MongoClient = MongoClient(MongoClientURI("mongodb://192.168.10.102:27017/recommender"))
}
...
object RealTime {
  // 定义常量和表名
  val MONGODB_RATING_COLLECTION = "rate"
  val STREAM_RECS = "recs" //推荐结果表
  val PRODUCT_RECS = "psg_recs" //相似度矩阵表

  val MAX_USER_RATING_NUM = 20 //最大推荐数量
  val MAX_SIM_PRODUCTS_NUM = 20 //相似商品选取多少个

  def main(args: Array[String]): Unit = {
    val config = Map(
      "spc" -> "local[*]",
      "spn" -> "OnlineRecommender",
      "mongo.uri" -> "mongodb://192.168.10.102:27017/recommender",
    ...
    val spark = SparkSession.builder().config(sparkConf).getOrCreate() //创建spark session
    val sc = spark.sparkContext
    val ssc = new StreamingContext(sc, Seconds(2)) //因为spark sessionn中没有Streaming Coentext，所以需要单独实例化；Seconds(2)：采集周期

    import spark.implicits._
    implicit val mongoConfig: MongoConfig = MongoConfig(config("mongo.uri"), config("mongo.db"))

    // 加载相似度矩阵。因为相似度矩阵是非常大的，所以可以广播出去，广播出去后每个executor都保存一个相似度矩阵的副本
    val simProductsMatrix = spark.read
      .option("uri", mongoConfig.uri) //定义uri
      .option("collection", PRODUCT_RECS) //定义collection
      .format("com.mongodb.spark.sql") //
      .load()
      .as[ProductRecs] //将读取好的数据转换为定义好的struct
      .rdd //可以不转化为RDD，就是代码写起来没那么方便
      // 为了后续查询相似度方便，把数据转换成map形式
      .map { item => (item.productId, item.recs.map(x => (x.productId, x.score)).toMap) }
      .collectAsMap() //将整个RDD转为map
    // 定义广播变量
    val simProcutsMatrixBC = sc.broadcast(simProductsMatrix)

    // 创建一个DStream
    val kafkaStream = KafkaUtils.createDirectStream[String, String]( //泛型是K, V的类型
      ssc,
      LocationStrategies.PreferConsistent, //自动选择服务器节点
      ConsumerStrategies.Subscribe[String, String](Array(config("kafka.topic")), kafkaParam) //订阅的topic
    )
