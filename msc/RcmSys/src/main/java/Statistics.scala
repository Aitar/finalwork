...
object Statistics{
  ...
  val RATE_MORE_PRODUCTS = "hot_psgs"   //评分个数（热度）
  val RATE_MORE_RECENTLY_PRODUCTS = "recent_hot"  //最近评分个数（最近热度）
  val AVERAGE_PRODUCTS = "avg_score"  //平均得分，优质商品统计
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 用spark sql去做不同的统计推荐
    // 1. 历史热门，按照评分个数统计，productId，count
